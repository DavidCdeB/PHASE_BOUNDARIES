...
# Load data:
y1, z1, x1  = np.loadtxt('/home/david/Trabajo/structures/SCRIPT_ON_ALL_THE_PHASES/Calcite_I_over_17_volumes/solid_1__xyz_sorted_as_P_wise.dat').T
#y_data, z_data, x_data  = np.loadtxt('./solid_1__xyz_sorted_as_P_wise.dat').T

y2, z2, x2  = np.loadtxt('/home/david/Trabajo/structures/SCRIPT_ON_ALL_THE_PHASES/Calcite_II_correct_description/solid_1__xyz_sorted_as_P_wise.dat').T
...
def within_tolerance(p1, p2):
    tol_z = 0.005
    tol_x = 20.0  # shouldn't this be 0.000005?
    tol_y = 0.005
...
#ax.scatter(T1, P1, color='r', marker='o', label='Calcite I (P,T) data for which $G^{I}(T,P)=G^{II}(T,P)$')
plt.scatter(T2, P2, color='b', marker='o', label='Calcite II (P,T) data \nfor which $G^{I}, T^{I}, P^{I} = G^{II}, T^{II}, P^{II}$')
#ax.scatter(T2, P2, color='b', marker='o', label='Calcite II (P,T) data for which $G^{I}(T,P)=G^{II}(T,P)$')
tics_shown =  [10, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250]
#plt.xticks(tics_shown)
#ax.set_xticks(tics_shown)
plt.grid()
#ax.grid()
#plt.legend()
...
fig.savefig("Matching_1_and_2_on_G_T_and_P.pdf",  bbox_inches='tight', pad_inches=0.3)#, tight_layout() )#, bbox_inches=bbox)
